    ...
    <div className={`relative ${className}`} onClick={() => { setNav(!nav); }}>
      ...
      {nav &&
        ...
        {genreList.map((item, index) => (
          <div key={index} className={`flex gap-2 items-center rounded-xl p-1 mb-4  text-black dark:text-white hover:scale-110 transition-all duration-300 cursor-pointer ${active === index ? "bg-slate-300 dark:bg-gray-700" : null}`} onClick={() => { setActive(index); setGenreId(item.id); setGameHeaderByGenreName(item.name); }}>
