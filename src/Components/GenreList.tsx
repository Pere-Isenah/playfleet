    ...
    <div className="p-3 pr-4">
      ...
      <div >
        {genreList.map((item, index) => (
          <div key={index} className={`flex gap-2 items-center rounded-xl p-2 mb-4  text-black dark:text-white hover:scale-110 transition-all duration-300 cursor-pointer ${active==index? "bg-slate-300 dark:bg-gray-700":null }`} onClick={()=>{setActive(index), selectedGenreId(item.id), selectedGenreName(item.name)}}>
