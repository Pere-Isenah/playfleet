...
const { genreId, gameHeaderByGenreName, allGameList } =
  ...
  // Assuming selectedGenre has an id property
  
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useGamesByGenreId({genreId: genreId }); // Pass genreId to the hook
  ...
  useEffect(() => {
    if (inView && hasNextPage) { // Check if there is a next page before fetching
      fetchNextPage(); // Call fetchNextPage when inView and hasNextPage are true
    }
